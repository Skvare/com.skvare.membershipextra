...
class CRM_Membershipextra_Form_Report_Member_SeparateMembershipamountTest extends CRM_Report_Form {
  ...
  public function membershipAmountNew() {
    $select = "SELECT DISTINCT {$this->_aliases['civicrm_line_item']}.id";

    $sql = "SELECT mt.name, SUM(ll.line_total) as membershipamount 
            FROM civicrm_line_item ll
            inner join civicrm_price_field pf on (pf.id= ll.price_field_id)
            inner join civicrm_price_field_value pfv on (pfv.id= ll.price_field_value_id)
            inner join civicrm_membership_type mt on (pfv.membership_type_id = mt.id)
            WHERE ll.id IN ({$select} {$this->_from} {$this->_where})
            GROUP BY mt.name order by mt.name asc";
    //echo '<pre>'; print_r($sql); echo '</pre>';
    $dao = CRM_Core_DAO::executeQuery($sql);
    $totalAmount = [];
    while ($dao->fetch()) {
      $totalAmount[$dao->name] = $dao->membershipamount;
    }
    $finalAmount = CRM_Utils_Money::format(array_sum($totalAmount));
    /*
    $pricefieldvalues = array_keys($totalAmount);
    $membershipTypes = CRM_Member_PseudoConstant::membershipType();
    if (!empty($pricefieldvalues)) {
      $pricefieldvalue = implode(',', $pricefieldvalues);

      $sql = "select id, membership_type_id from civicrm_price_field_value where id IN({$pricefieldvalue})";
      $dao = CRM_Core_DAO::executeQuery($sql);
      $priceMembership= array();
      while ($dao->fetch()) {
        $priceMembership[$dao->id] = $dao->membership_type_id;
      }
    }
    */
    $details = [];
    ...
    $html = '<table>';
    foreach ($totalAmount as $key => &$values) {
      //$membershipTypeId = $priceMembership[$key];
      //$membershipType = $membershipTypes[$membershipTypeId];
      $values = CRM_Utils_Money::format($values);
      $html .= "<tr><td style='width: 32%;'>{$key}</td><td>{$values}</td></tr>";
    }
    $html .= "<tr><td style='width: 32%;'>Total Amount</td><td>{$finalAmount}</td></tr>";
    $html .= '</table>';
  ...
  public function statistics(&$rows) {
    ...
    $financialTypeDetails = $this->membershipAmountNew();

    $statistics['counts']['summary'] = [
      'title' => ts('Membership Type Payment Breakdown'),
      'value' => $financialTypeDetails,
      'type' => CRM_Utils_Type::T_STRING,
    ];
